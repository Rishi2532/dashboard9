...
interface CombinedSchemesWidgetProps {
  schemes: any[];
  selectedRegion?: string;
}

const CombinedSchemesWidget: React.FC<CombinedSchemesWidgetProps> = ({
  schemes,
  selectedRegion = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet("Combined Schemes Analysis");
      ...
      const filename = `Combined_Schemes_Analysis${regionText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting combined schemes to Excel:", error);
    }
  };
  ...
  const normalizeStatus = (status: string) => {
    if (!status) return '';
    return status.toLowerCase().replace(/[-_\s]+/g, ' ').trim();
  };

  const getStatusColor = (status: string) => {
    const normalized = normalizeStatus(status);
    if (normalized === 'fully completed' || normalized === 'completed') {
  ...
  // Calculate summary statistics using normalized status comparison
  const fullyCompleted = schemes.filter(s => {
    const normalized = normalizeStatus(s.fully_completion_scheme_status || '');
    return normalized === 'fully completed' || normalized === 'completed';
  }).length;

  const inProgress = schemes.filter(s => {
    const normalized = normalizeStatus(s.fully_completion_scheme_status || '');
    return normalized === 'in progress';
  }).length;

  return (
    <div className="combined-schemes-widget mt-2 mb-2">
      <div className="bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-4 py-3 bg-gray-100 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-medium text-gray-700">
              {selectedRegion !== "all" ? `${selectedRegion} Region - Schemes Analysis` : "Schemes Analysis"}
            </h3>
            <Button
              size="sm"
          ...
          <div className="mt-2 flex gap-4 text-xs text-gray-600">
            <span data-testid="text-total-schemes">
              Total: {schemes.length} schemes
            </span>
            <span data-testid="text-fully-completed-count" className="text-green-700">
              Fully Completed: {fullyCompleted}
            </span>
            <span data-testid="text-in-progress-count" className="text-yellow-700">
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center">
                  ...
                  >
                    {getStatusDisplayText(scheme.fully_completion_scheme_status)}
                  </span>
                </div>
              </div>
...
export default CombinedSchemesWidget;