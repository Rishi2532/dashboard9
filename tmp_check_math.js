...
async function checkLpcdMath() {
  ...
  const result = await pool.query(query, [scheme_id, village_name, dates]);
  ...
  const numericValues = uniqueValues
    .map(v => (v === 'NaN' || v === null || v === '') ? null : parseFloat(v))
...
checkLpcdMath().catch(console.error).finally(() => pool.end());
