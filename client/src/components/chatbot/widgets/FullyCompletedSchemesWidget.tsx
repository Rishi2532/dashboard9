...
interface FullyCompletedSchemesWidgetProps {
  schemes: any[];
  selectedRegion?: string;
}

const FullyCompletedSchemesWidget: React.FC<FullyCompletedSchemesWidgetProps> = ({ 
  schemes, 
  selectedRegion = "all" 
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet("Fully Completed Schemes");
      ...
      const filename = `Fully_Completed_Schemes${regionText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting fully completed schemes to Excel:", error);
    }
  };

  return (
    <div className="fully-completed-schemes-widget mt-2 mb-2">
      <div className="bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
        <div className="px-4 py-3 bg-gray-100 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-medium text-gray-700">
              {selectedRegion !== "all" ? `${selectedRegion} Region - Fully Completed Schemes` : "Fully Completed Schemes"}
            </h3>
            <Button 
              size="sm" 
...
export default FullyCompletedSchemesWidget;