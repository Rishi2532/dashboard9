...
interface AverageBelowChlorineWidgetProps {
  esrs: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const AverageBelowChlorineWidget: React.FC<AverageBelowChlorineWidgetProps> = ({ 
  esrs, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = esrs.map((esr: any) => ({
        ...
        "Status": "Average Below Optimal Chlorine (<0.2 mg/L)",
        "Last Updated": new Date().toLocaleDateString("en-IN"),
      }));

      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Avg Below Chlorine");
      ...
      const filename = `ESRs_Avg_Below_Chlorine${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting average below chlorine to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - ESRs with Average Below Optimal Chlorine (<0.2 mg/L)`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - ESRs with Average Below Optimal Chlorine (<0.2 mg/L)`;
    }
    return "ESRs with Average Below Optimal Chlorine (<0.2 mg/L)";
  };
  ...
  return (
    <div className="average-below-chlorine-widget mt-2 mb-2">
...
export default AverageBelowChlorineWidget;
