...
function replaceExact(oldStr, newStr, requiredCount = 1) {
    let count = 0;
    // Normalize both strings and file content to UNIX newlines for reliable matching
    ...
    while (idx !== -1) {
        ...
        count++;
        idx = content.indexOf(oldStr, idx + newStr.length);
    }
    if (count !== requiredCount) {
        console.error("Failed exact match! Expected " + requiredCount + " but found " + count + " for:\n" + oldStr.substring(0, 50) + "...");
    } else {
        console.log("Successfully replaced " + count + " occurrence(s) for:", oldStr.substring(0, 50) + "...");
    }
}

// 1. Regional Stats CTE
...
replaceExact(oldRegional, newRegional, 1);

// 2. Base Query (for details and details-export)
...
replaceExact(oldBaseQuery, newBaseQuery, 2);

// 3. Case conditions
replaceExact('WHERE lpcd_value IS NOT NULL AND lpcd_value::numeric > 55', 'WHERE lpcd_value IS NOT NULL AND lpcd_value::numeric >= 55', 2);
replaceExact('WHERE lpcd_value IS NOT NULL AND lpcd_value::numeric > 0 AND lpcd_value::numeric <= 55', 'WHERE ((water_value IS NOT NULL AND water_value::numeric > 0) OR (lpcd_value IS NOT NULL AND lpcd_value::numeric > 0)) AND (lpcd_value IS NULL OR lpcd_value::numeric < 55)', 2);
replaceExact('WHERE lpcd_value IS NULL OR lpcd_value::numeric = 0', 'WHERE (water_value IS NULL OR water_value::numeric = 0) AND (lpcd_value IS NULL OR lpcd_value::numeric = 0)', 2);

// 4. Division Summary CTE
...
replaceExact(oldDivSumCTE, newDivSumCTE, 1);

// 5. Division Details CTE
...
replaceExact(oldDivDetailsCTE, newDivDetailsCTE, 1);

// 6. Division Details Export CTE
...
replaceExact(oldDivDetailsExportCTE, newDivDetailsExportCTE, 1);
