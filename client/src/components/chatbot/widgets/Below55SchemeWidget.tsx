...
interface Below55SchemeWidgetProps {
  schemes: any[];
  selectedRegion?: string;
}

const Below55SchemeWidget: React.FC<Below55SchemeWidgetProps> = ({ 
  schemes, 
  selectedRegion = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet("Schemes Below 55 LPCD");
      ...
      const filename = `Schemes_Below_55_LPCD${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting schemes below 55 LPCD to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - Schemes Below 55 LPCD`;
    }
    return "Schemes Below 55 LPCD";
  };

  return (
    <div className="below-55-scheme-widget mt-2 mb-2">
          ...
          {schemesToDisplay.map((scheme, index) => (
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <h4 className="text-sm font-medium text-orange-700" data-testid={`text-scheme-name-${index}`}>
                    {scheme.scheme_name}
                  </h4>
...
export default Below55SchemeWidget;
