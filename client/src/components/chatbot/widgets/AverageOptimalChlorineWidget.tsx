...
interface AverageOptimalChlorineWidgetProps {
  esrs: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const AverageOptimalChlorineWidget: React.FC<AverageOptimalChlorineWidgetProps> = ({ 
  esrs, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = esrs.map((esr: any) => ({
        ...
        "Status": "Average Optimal Chlorine (0.2-0.5 mg/L)",
        "Last Updated": new Date().toLocaleDateString("en-IN"),
      }));

      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Avg Optimal Chlorine");
      ...
      const filename = `ESRs_Avg_Optimal_Chlorine${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting average optimal chlorine to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - ESRs with Average Optimal Chlorine (0.2-0.5 mg/L)`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - ESRs with Average Optimal Chlorine (0.2-0.5 mg/L)`;
    }
    return "ESRs with Average Optimal Chlorine (0.2-0.5 mg/L)";
  };
  ...
  return (
    <div className="average-optimal-chlorine-widget mt-2 mb-2">
...
export default AverageOptimalChlorineWidget;
