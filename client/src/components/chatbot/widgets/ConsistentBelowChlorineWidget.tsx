...
interface ConsistentBelowChlorineWidgetProps {
  selectedRegion?: string;
  selectedScheme?: string;
}

const ConsistentBelowChlorineWidget: React.FC<ConsistentBelowChlorineWidgetProps> = ({ 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  const { data, isLoading, error } = useQuery({
    queryKey: ['/api/category-data/chlorine/consistent-below', selectedRegion, selectedScheme],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/category-data/chlorine/consistent-below?${params.toString()}`);
      if (!response.ok) {
        throw new Error(`Failed to fetch consistent below chlorine data: ${response.status}`);
      }
  ...
  const handleExportToExcel = async () => {
    ...
    try {
      const exportData = data.data.map((esr: any) => ({
        ...
        "Status": "Below Optimal (<0.2 mg/L) for all 7 days"
      }));

      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Consistent Below Chlorine");
      ...
      link.href = url;
      link.download = `consistent_below_chlorine_${new Date().toISOString().split("T")[0]}.xlsx`;
      link.click();
    ...
    <div className="space-y-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-red-600 dark:text-red-400">
            ❌ Consistent Below Optimal Chlorine (&lt;0.2 mg/L)
          </h3>
          <p className="text-sm text-gray-600 dark:text-gray-400">
            ESRs with below optimal chlorine levels for all 7 days
          </p>
        </div>
        <Button
          ...
          size="sm"
          className="gap-2"
          data-testid="button-export-consistent-below-chlorine"
        >
      ...
      <div className="grid grid-cols-1 gap-4">
        <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-lg border border-red-200 dark:border-red-800">
          <div className="text-3xl font-bold text-red-600 dark:text-red-400" data-testid="text-count-consistent-below-chlorine">
            {data?.count || 0}
          </div>
          <div className="text-sm text-gray-600 dark:text-gray-400">
            ESRs with consistent below optimal chlorine
          </div>
        </div>
...
export default ConsistentBelowChlorineWidget;
