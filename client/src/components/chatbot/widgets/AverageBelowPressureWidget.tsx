...
interface AverageBelowPressureWidgetProps {
  esrs: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const AverageBelowPressureWidget: React.FC<AverageBelowPressureWidgetProps> = ({ 
  esrs, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = esrs.map((esr: any) => ({
        ...
        "Status": "Average Below Optimal Pressure (<0.2 bar)",
        "Last Updated": new Date().toLocaleDateString("en-IN"),
      }));

      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Avg Below Pressure");
      ...
      const filename = `ESRs_Avg_Below_Pressure${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting average below pressure to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - ESRs with Average Below Optimal Pressure (<0.2 bar)`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - ESRs with Average Below Optimal Pressure (<0.2 bar)`;
    }
    return "ESRs with Average Below Optimal Pressure (<0.2 bar)";
  };
  ...
  return (
    <div className="average-below-pressure-widget mt-2 mb-2">
...
export default AverageBelowPressureWidget;
