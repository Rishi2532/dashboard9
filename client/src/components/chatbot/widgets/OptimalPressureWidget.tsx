...
interface OptimalPressureWidgetProps {
  esrs: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const OptimalPressureWidget: React.FC<OptimalPressureWidgetProps> = ({ 
  esrs, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = esrs.map((esr: any) => ({
        ...
        "Connection Status": esr.pressure_connected || "Unknown",
        "Last Updated": new Date().toLocaleDateString("en-IN"),
      }));

      // Create workbook and worksheet
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("ESRs with Optimal Pressure");
      ...
      // Style header row (blue theme for optimal pressure)
      ...
      const filename = `ESRs_Optimal_Pressure${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting ESRs with optimal pressure to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - ESRs with Optimal Pressure`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - ESRs with Optimal Pressure`;
    }
    return "ESRs with Optimal Pressure";
  };

  return (
    <div className="optimal-pressure-widget mt-2 mb-2">
            ...
            >
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                <div className="text-xs" data-testid={`text-latest-pressure-${index}`}>
                  <span className="text-gray-500">Latest Pressure:</span> {Number(esr.pressure_value_7 || 0).toFixed(2)} bar
                </div>
                ...
                <div className="text-xs" data-testid={`text-connection-${index}`}>
                  <span className="text-gray-500">Connection:</span> {esr.pressure_connected || 'Unknown'}
                </div>
              </div>
            </div>
          ))}
...
export default OptimalPressureWidget;