...
interface ConsistentAbovePressureWidgetProps {
  selectedRegion?: string;
  selectedScheme?: string;
}

const ConsistentAbovePressureWidget: React.FC<ConsistentAbovePressureWidgetProps> = ({ 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  const { data, isLoading, error } = useQuery({
    queryKey: ['/api/category-data/pressure/consistent-above', selectedRegion, selectedScheme],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/category-data/pressure/consistent-above?${params.toString()}`);
      if (!response.ok) {
        throw new Error(`Failed to fetch consistent above pressure data: ${response.status}`);
      }
  ...
  const handleExportToExcel = async () => {
    ...
    try {
      const exportData = data.data.map((esr: any) => ({
        ...
        "Status": "Above Optimal (>0.7 bar) for all 7 days"
      }));

      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Consistent Above Pressure");
      ...
      link.href = url;
      link.download = `consistent_above_pressure_${new Date().toISOString().split("T")[0]}.xlsx`;
      link.click();
    ...
    <div className="space-y-4 p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-amber-600 dark:text-amber-400">
            ⚠️ Consistent Above Optimal Pressure (&gt;0.7 bar)
          </h3>
          <p className="text-sm text-gray-600 dark:text-gray-400">
            ESRs with above optimal pressure levels for all 7 days
          </p>
        </div>
        <Button
          ...
          size="sm"
          className="gap-2"
          data-testid="button-export-consistent-above-pressure"
        >
      ...
      <div className="grid grid-cols-1 gap-4">
        <div className="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
          <div className="text-3xl font-bold text-amber-600 dark:text-amber-400" data-testid="text-count-consistent-above-pressure">
            {data?.count || 0}
          </div>
          <div className="text-sm text-gray-600 dark:text-gray-400">
            ESRs with consistent above optimal pressure
          </div>
        </div>
...
export default ConsistentAbovePressureWidget;
