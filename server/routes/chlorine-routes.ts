  ...
  try {
    ...
    try {
      ...
      for (const region of regions) {
        ...
        const offlineQuery = `
          SELECT 
            SUM(CASE WHEN cs.chlorine_status IN ('Offline', 'offline') AND (wc.water_value_day7 IS NULL OR wc.water_value_day7 = 0) THEN 1 ELSE 0 END) as offline_with_no_water_count,
            SUM(CASE WHEN cs.chlorine_status IN ('Offline', 'offline') AND wc.water_value_day7 > 0 THEN 1 ELSE 0 END) as offline_with_water_count,
        ...
        const offline_with_no_water = parseInt(offlineResult.rows[0]?.offline_with_no_water_count || '0');
        const offline_with_water = parseInt(offlineResult.rows[0]?.offline_with_water_count || '0');
        ...
        comparisonData.push({
          region,
          offline: offline,
          offline_with_no_water: offline_with_no_water,
          offline_with_water: offline_with_water,
  ...
  try {
    ...
    try {
      ...
      if (category.startsWith('weekly_')) {
        ...
        const avgCalc = `SUM(COALESCE(NULLIF(TRIM(lpcd_value::text), '')::numeric, 0)) / ${dateList.length}`;
            ...
            village_stats AS (
                SELECT
                    ...
                    CASE 
                      ...
                      THEN ROUND((SUM(COALESCE(NULLIF(TRIM(water_value::text), '')::numeric, 0)) / ${dateList.length}), 2)
                      ELSE 0 
...
router.get("/overall-region-comparison/export/:category", async (req, res) => {
  try {
    ...
    try {
      ...
      } else if (['offline', 'offline-with-no-water', 'offline-with-water', 'online_no_water'].includes(category)) {
        worksheet.columns = [
          { header: 'Region', key: 'region', width: 20 },
          { header: 'Circle', key: 'circle', width: 20 },
          { header: 'Division', key: 'division', width: 20 },
  ...
  try {
    ...
    try {
      ...
      const query = `
        ...
        unique_schemes AS (
          SELECT DISTINCT scheme_id, block
      ...
      switch (statisticType) {
        ...
        case 'schemesBelow55': // Renamed from 'schemes-below-55' to match existing frontend calls
          query = `
            WITH latest_data AS (${baseQuery})
            SELECT * FROM latest_data
            WHERE ((water_value IS NOT NULL AND water_value::numeric > 0) OR (lpcd_value IS NOT NULL AND lpcd_value::numeric > 0)) AND (lpcd_value IS NULL OR lpcd_value::numeric < 55)
            ORDER BY region, scheme_id
          `;
          break;
        ...
        case 'schemesNoSupply': // Renamed from 'schemes-no-supply' to match existing frontend calls
          query = `
            WITH latest_data AS (${baseQuery})
            SELECT * FROM latest_data
            WHERE (water_value IS NULL OR water_value::numeric = 0) AND (lpcd_value IS NULL OR lpcd_value::numeric = 0)
            ORDER BY region, scheme_id
          `;
          break;
        ...
        case 'weekly_total_schemes':
          ...
          const avgCalc2 = `SUM(COALESCE(NULLIF(TRIM(lpcd_value::text), '')::numeric, 0)) / ${dateList2.length}`;
          ...
          query = `
            ...
            SELECT 
              ...
              ROUND((SUM(COALESCE(NULLIF(TRIM(water_value::text), ''), '0')::numeric) / ${dateList2.length}), 2) as water_value,
...
router.get("/scheme-lpcd/details-export/:statisticType", async (req, res) => {
  try {
    ...
    try {
      ...
      const baseQuery = `
        SELECT DISTINCT ON (scheme_name, region, scheme_id, block)
            region, circle, division, sub_division, block,
      ...
      switch (statisticType) {
        ...
        case 'weekly_total_schemes':
          ...
          const avgCalc3 = `SUM(COALESCE(NULLIF(TRIM(lpcd_value::text), '')::numeric, 0)) / ${dateList3.length}`;
          ...
          query = `
            ...
            SELECT 
              ...
              ROUND((SUM(COALESCE(NULLIF(TRIM(water_value::text), ''), '0')::numeric) / ${dateList3.length}), 2) as water_value,
              MAX(data_date) as data_date
            FROM all_schemes s
            LEFT JOIN deduplicated d ON s.scheme_id = d.scheme_id
  ...
  try {
    ...
    try {
      ...
      if (category.startsWith('weekly_')) {
        ...
        switch (category) {
          case 'above_55':
            metricCondition = 'AND lpcd_value >= 55';
            break;
          case 'below_55':
            metricCondition = 'AND lpcd_value > 0 AND lpcd_value < 55';
            break;
          case 'with_water':
            metricCondition = 'AND lpcd_value > 0';
            break;
          case 'no_water':
            metricCondition = 'AND (lpcd_value IS NULL OR lpcd_value = 0)';
            break;
          default:
            metricCondition = '';
        }
        ...
        query = `
          WITH ranked_history AS (
            SELECT 
              h.scheme_id, h.block, h.scheme_name, h.lpcd_value, h.water_value,
              h.total_population, h.total_villages, h.villages_above_55, h.villages_below_55, h.villages_zero_supply,
      ...
      if (category.startsWith('weekly_')) {
        ...
        scheme_stats AS (
          SELECT
              ...
              ROUND((SUM(COALESCE(NULLIF(TRIM(lpcd_value:: text), '')::numeric, 0)) / ${dateList.length}), 2) as lpcd_value,
              ROUND((SUM(COALESCE(NULLIF(TRIM(water_value:: text), '')::numeric, 0)) / ${dateList.length}), 2) as water_value,
              MAX(data_date) as data_date
          FROM(
        ...
        ORDER BY ss.region, ss.scheme_id
            `;
      } else {
