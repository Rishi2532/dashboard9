...
interface BelowPressureWidgetProps {
  esrs: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const BelowPressureWidget: React.FC<BelowPressureWidgetProps> = ({ 
  esrs, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet("ESRs with Below Pressure");
      ...
      // Style header row (orange theme for low pressure)
      ...
      const filename = `ESRs_Below_Pressure${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting ESRs with below pressure to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - ESRs with Below Pressure`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - ESRs with Below Pressure`;
    }
    return "ESRs with Below Pressure";
  };

  return (
    <div className="below-pressure-widget mt-2 mb-2">
...
export default BelowPressureWidget;