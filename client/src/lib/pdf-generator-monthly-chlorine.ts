...
export interface MonthlyChlorineData {
  caseType: string;
  summary: {
  ...
  records: any[];
  chlorineCommissionedSchemes: any[];
  lpcdLast14Days?: any[];
  chlorineHighlights?: any[];
...
export async function generateMonthlyChlorineReportPDF(data: MonthlyChlorineData) {
  ...
  renderLpcdTable(data.chlorineCommissionedSchemes, "2. Monthly Chlorine Integrated Schemes Data", "Chlorine data spanning the valid dates for commissioned schemes, displayed in 2-week sub-tables. Values between 0.2 and 0.5 are highlighted green, < 0.2 in yellow, and > 0.5 in red.");
  renderLpcdTable(data.chlorineHighlights || [], "3. Highlights - Consistent Optimal Chlorine", "Schemes that achieved consistent optimal chlorine (0.2–0.5 mg/L) every single recorded day throughout the period.");
