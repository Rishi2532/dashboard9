...
interface AbovePressureWidgetProps {
  esrs: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const AbovePressureWidget: React.FC<AbovePressureWidgetProps> = ({ 
  esrs, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet("ESRs with Above Pressure");
      ...
      // Style header row (red theme for high pressure)
      ...
      const filename = `ESRs_Above_Pressure${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting ESRs with above pressure to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - ESRs with Above Pressure`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - ESRs with Above Pressure`;
    }
    return "ESRs with Above Pressure";
  };

  return (
    <div className="above-pressure-widget mt-2 mb-2">
...
export default AbovePressureWidget;