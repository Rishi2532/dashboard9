...
export async function generateMonthlyChlorineReportPDF(data: MonthlyChlorineData) {
  ...
  const tocItems = [
    { text: "Monthly Chlorine Integrated Schemes Data", margin: [0, 20, 0, 20], fontSize: 18 },
  ];
  content.push({
    ul: tocItems,
    ...
    tablesToRender.forEach((tableObj, tableIndex) => {
      ...
      if (actualHeaders.length === 0 || rowsToRender.length === 0) {
        ...
        lpcdBody.push(actualHeaders.map(key => {
          ...
          if (keyStr.includes("0.2-0.5")) {
            fillColor = themeColors.green;
          } else if (keyStr.includes("< 0.2") || keyStr.includes("> 0.5")) {
  ...
  renderLpcdTable(data.chlorineCommissionedSchemes, "Monthly Chlorine Integrated Schemes Data", "Chlorine data spanning the valid dates for commissioned schemes, displayed in 2-week sub-tables. Values between 0.2 and 0.5 are highlighted green, < 0.2 in yellow, and > 0.5 in red.", false);
