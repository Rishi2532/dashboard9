...
interface ConsistentAbove55LpcdWidgetProps {
  villages: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const ConsistentAbove55LpcdWidget: React.FC<ConsistentAbove55LpcdWidgetProps> = ({ 
  villages, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = villages.map((village: any) => ({
        ...
        "Status": "Consistently Above 55 LPCD",
        "Last Updated": new Date().toLocaleDateString("en-IN"),
      }));

      // Create workbook and worksheet
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Villages Consistently Above 55 LPCD");
      ...
      // Style header row (dark green theme for excellent LPCD)
      ...
      const filename = `Villages_Consistently_Above_55_LPCD${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting villages consistently above 55 LPCD to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - Villages Consistently Above 55 LPCD`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - Villages Consistently Above 55 LPCD`;
    }
    return "Villages Consistently Above 55 LPCD";
  };

  return (
    <div className="consistent-above-55-lpcd-widget mt-2 mb-2">
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center">
                  <span 
                    className="text-xs px-2 py-1 rounded-full font-medium bg-emerald-100 text-emerald-800"
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                <div className="text-xs" data-testid={`text-consistent-days-${index}`}>
                  <span className="text-gray-500">Above 55:</span> 7/7 days
                </div>
              </div>
            </div>
          ))}
...
export default ConsistentAbove55LpcdWidget;