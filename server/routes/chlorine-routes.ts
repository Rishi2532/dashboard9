...
function getLastCompletedISOWeekInfo() {
...
router.get("/weekly-lpcd/stats", async (req, res) => {
  try {
    const weekInfo = getLastCompletedISOWeekInfo();
    console.log("Weekly LPCD Stats Request for:", weekInfo);
  ...
  try {
    ...
    try {
      ...
      for (const region of regions) {
        // Offline sensors from communication_status
        const offlineQuery = `
          SELECT COUNT(*) as count 
          FROM communication_status 
          WHERE region = $1 
          AND (chlorine_status = 'Offline' OR chlorine_status = 'offline')
          AND chlorine_connected = 'Connected'
          ${schemeIdFilter}
        `;
      ...
      if (category.startsWith('weekly_')) {
        ...
        switch (category) {
          ...
          case 'all_sensors':
            ...
            LEFT JOIN chlorine_data cd ON cs.scheme_id = cd.scheme_id AND cs.scheme_name = cd.scheme_name AND cs.village_name = cd.village_name AND cs.esr_name = cd.esr_name
            WHERE (
              (cs.chlorine_connected = 'Connected' AND (cs.chlorine_status = 'Offline' OR cs.chlorine_status = 'offline'))
              OR
              (cd.chlorine_value_7 IS NOT NULL)
            )
            ${region ? 'AND cs.region = $1' : ''}
            ${schemeIdFilterGeneric.replace('scheme_id', 'cs.scheme_id')}
            ORDER BY cs.region, cs.division, cs.village_name
      ...
      const query = `
        WITH latest_scheme_data AS (
          SELECT DISTINCT ON (region, scheme_id, block)
            region, scheme_id, scheme_name, block, 
        ...
        scheme_stats AS (
          SELECT 
            ...
            COUNT(DISTINCT CASE 
              WHEN lpcd_value IS NOT NULL AND lpcd_value::numeric > 0 AND lpcd_value::numeric <= 55 
              THEN scheme_id || '-' || COALESCE(block, '')
            END) as schemes_below_55,
            COUNT(DISTINCT CASE 
              WHEN lpcd_value IS NULL OR lpcd_value::numeric = 0 
              THEN scheme_id || '-' || COALESCE(block, '')
        ...
        history_stats AS (
          ...
          FROM (
            SELECT DISTINCT ON (region, scheme_id, block, data_date)
              region, scheme_id, block, data_date, lpcd_value
            FROM scheme_lpcd_data_history
            WHERE region IS NOT NULL 
              AND data_date IS NOT NULL
        ...
        SELECT 
          ss.region,
          COALESCE(ss.total_schemes, 0) as total_schemes,
          COALESCE(ss.total_population, 0) as total_population,
          COALESCE(ss.total_villages, 0) as total_villages,
          COALESCE(ss.schemes_above_55, 0) as schemes_above_55,
          COALESCE(ss.schemes_below_55, 0) as schemes_below_55,
          COALESCE(ss.schemes_no_supply, 0) as schemes_no_supply,
          COALESCE(cb.below_55_3days, 0) as below_55_3days,
          COALESCE(cb.below_55_7days, 0) as below_55_7days,
          COALESCE(cb.below_55_30days, 0) as below_55_30days
        FROM scheme_stats ss
        LEFT JOIN consecutive_below_55 cb ON ss.region = cb.region
...
router.get("/scheme-lpcd/region-comparison", async (req, res) => {
  try {
    ...
    try {
      // Get latest data date first
      const latestDateResult = await client.query(`
        SELECT data_date as latest_date 
        FROM scheme_lpcd_data_history 
        WHERE data_date IS NOT NULL
        ORDER BY 
          CASE 
      ...
      const query = `
        ...
        SELECT 
          ...
          END) as below_55,
          COUNT(DISTINCT CASE 
            WHEN water_value IS NOT NULL AND water_value::numeric > 0 
            THEN scheme_id || '-' || COALESCE(block, '')
          END) as with_water,
          COUNT(DISTINCT CASE 
            WHEN water_value IS NULL OR water_value::numeric = 0 
            THEN scheme_id || '-' || COALESCE(block, '')
          END) as no_water
        FROM latest_scheme_data
        GROUP BY region
    ...
    try {
      ...
      if (category.startsWith('weekly_')) {
        ...
        SELECT 
          ...
          lv.water_value as latest_water_value,
          lv.data_date as latest_date
        FROM history_stats hs
