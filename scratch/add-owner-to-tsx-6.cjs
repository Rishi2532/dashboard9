...
let currentTableHeadColor = '';
let currentTableCellColor = '';
...
for(let i=0; i < lines.length; i++) {
    ...
    if (line.includes('Scheme Name') && outLines[outLines.length - 2] && outLines[outLines.length - 2].includes('<TableHead')) {
        ...
        // append Owner
        outLines.push(                                  <TableHead className="font-semibold text-[11px] uppercase tracking-wider text-\ !px-4 !py-3.5 w-[100px] border-r border-white/10 text-center">);
        outLines.push(                                    Owner);
    ...
    if (line.includes('{sensor.scheme_name}') && outLines[outLines.length - 2] && outLines[outLines.length - 2].includes('<TableCell')) {
        ...
        outLines.push(                                      <TableCell className="!px-4 !py-3 text-[12px] text-center text-slate-700 dark:text-slate-300 border-r border-\-100/80 dark:border-\-900/60 font-medium">);
...
console.log('TSX successfully replaced line-by-line');
