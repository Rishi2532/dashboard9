...
interface AverageAbove55LpcdWidgetProps {
  villages: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const AverageAbove55LpcdWidget: React.FC<AverageAbove55LpcdWidgetProps> = ({ 
  villages, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = villages.map((village: any) => ({
        ...
        "Status": "Average Above 55 LPCD",
        "Last Updated": new Date().toLocaleDateString("en-IN"),
      }));

      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Avg Above 55 LPCD");
      ...
      const filename = `Villages_Avg_Above_55_LPCD${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting average above 55 LPCD to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - Villages with Average LPCD Above 55`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - Villages with Average LPCD Above 55`;
    }
    return "Villages with Average LPCD Above 55";
  };
  ...
  return (
    <div className="average-above-55-lpcd-widget mt-2 mb-2">
...
export default AverageAbove55LpcdWidget;
