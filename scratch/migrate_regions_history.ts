...
async function main() {
  ...
  try {
    console.log("Starting database transaction for region history migration...");
    await pool.query("BEGIN");

    // 1. Delete all region history records uploaded today (June 18, 2026)
    const deleteRes = await pool.query(
      `DELETE FROM region_history 
       WHERE uploaded_at >= '2026-06-18T00:00:00.000Z' AND uploaded_at <= '2026-06-18T23:59:59.999Z'`
    );
    console.log(`Deleted ${deleteRes.rowCount} region_history records uploaded today.`);

    // 2. May 4, 2026 Baseline data (stored as May 1st baseline: 2026-04-30T18:30:00.000Z)
    const may4Data = [
    ...
    // 3. June 2, 2026 Baseline data (stored as June 1st baseline: 2026-05-31T18:30:00.000Z)
    const june2Data = [
    ...
    // 4. June 10, 2026 data (stored with June 10 baseline date)
    const june10Data = [
      { name: "Nagpur", schemes: 25, villages: 190, villages_comp: 159, esr: 280, esr_comp: 232, partial_esr: 48, fm: 280, rca: 248, pt: 252 },
    ...
    // 5. June 15, 2026 data (stored with June 15 baseline date)
    const june15Data = [
      { name: "Nagpur", schemes: 25, villages: 190, villages_comp: 159, esr: 280, esr_comp: 232, partial_esr: 48, fm: 280, rca: 248, pt: 252 },
