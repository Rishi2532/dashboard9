...
interface AboveChlorineWidgetProps {
  esrs: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const AboveChlorineWidget: React.FC<AboveChlorineWidgetProps> = ({ 
  esrs, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = esrs.map((esr: any) => ({
        ...
        "Chlorine Status": "Over-Chlorinated (>0.5 mg/L)",
      ...
      const worksheet = workbook.addWorksheet("ESRs with Above Chlorine");
      ...
      // Style header row (red theme for over-chlorination)
      ...
      const filename = `ESRs_Above_Chlorine${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting ESRs with above chlorine to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - ESRs with Above Chlorine`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - ESRs with Above Chlorine`;
    }
    return "ESRs with Above Chlorine";
  };

  return (
    <div className="above-chlorine-widget mt-2 mb-2">
...
export default AboveChlorineWidget;