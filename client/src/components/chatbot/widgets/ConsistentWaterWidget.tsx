...
interface ConsistentWaterWidgetProps {
  villages: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const ConsistentWaterWidget: React.FC<ConsistentWaterWidgetProps> = ({ 
  villages, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = villages.map((village: any) => ({
        ...
        "Status": "Consistent Water Supply",
        "Last Updated": new Date().toLocaleDateString("en-IN"),
      }));

      // Create workbook and worksheet
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Villages with Consistent Water");
      ...
      // Style header row (green theme for consistent water)
      ...
      const filename = `Villages_Consistent_Water${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting villages with consistent water to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - Villages with Consistent Water`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - Villages with Consistent Water`;
    }
    return "Villages with Consistent Water";
  };

  return (
    <div className="consistent-water-widget mt-2 mb-2">
          ...
          {villagesToDisplay.map((village, index) => (
            ...
            >
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                <div className="text-xs" data-testid={`text-water-days-${index}`}>
                  <span className="text-gray-500">Water Days:</span> 7/7 (All week)
                </div>
                <div className="text-xs" data-testid={`text-latest-reading-${index}`}>
                  <span className="text-gray-500">Latest Reading:</span> {village.water_value_day7 || 0} LL
                </div>
              </div>
            </div>
          ))}
...
export default ConsistentWaterWidget;