...
async function check() {
    try {
        // Find a scheme that has multiple records
        const schemes = await pool.query('SELECT scheme_name, count(*) FROM scheme_lpcd_data_history GROUP BY scheme_name HAVING count(*) > 3 LIMIT 5');
        console.log('Schemes with multiple records:', schemes.rows);

        if (schemes.rows.length > 0) {
            const name = schemes.rows[0].scheme_name;
            console.log(`\nRecords for scheme: ${name}`);
        ...
        `, [name]);
            records.rows.forEach(r => console.log(`Date: ${r.data_date} -> Parsed: ${r.parsed_date ? r.parsed_date.toISOString().split('T')[0] : 'FAILED'}, LPCD: ${r.lpcd_value}`));
        }
