...
interface AverageOptimalPressureWidgetProps {
  esrs: any[];
  selectedRegion?: string;
  selectedScheme?: string;
}

const AverageOptimalPressureWidget: React.FC<AverageOptimalPressureWidgetProps> = ({ 
  esrs, 
  selectedRegion = "all",
  selectedScheme = "all"
}) => {
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const exportData = esrs.map((esr: any) => ({
        ...
        "Status": "Average Optimal Pressure (0.2-0.7 bar)",
        "Last Updated": new Date().toLocaleDateString("en-IN"),
      }));

      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet("Avg Optimal Pressure");
      ...
      const filename = `ESRs_Avg_Optimal_Pressure${scopeText}_${new Date().toLocaleDateString("en-IN").replace(/\//g, "-")}.xlsx`;
      ...
      console.error("Error exporting average optimal pressure to Excel:", error);
    }
  };

  const getScopeTitle = () => {
    if (selectedScheme && selectedScheme !== "all") {
      return `${selectedScheme} - ESRs with Average Optimal Pressure (0.2-0.7 bar)`;
    } else if (selectedRegion && selectedRegion !== "all") {
      return `${selectedRegion} Region - ESRs with Average Optimal Pressure (0.2-0.7 bar)`;
    }
    return "ESRs with Average Optimal Pressure (0.2-0.7 bar)";
  };
  ...
  return (
    <div className="average-optimal-pressure-widget mt-2 mb-2">
...
export default AverageOptimalPressureWidget;
